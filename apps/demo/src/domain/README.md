...
`WeeklyScheduleEditor` — are **not** part of `@sereno/ui`. They encode Sereno's
...
on top of the DS — compose `@sereno/ui` primitives (`Card`, `Badge`, `Avatar`,
...
`apps/demo/app/globals.css` (it moved out of `@sereno/ui/styles.css` with the
component).
