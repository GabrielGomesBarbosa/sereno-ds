...
- **No UI base library.** 25 of the 27 components in `src/components/` are ported
  1:1 from the approved Design System (`FileUpload` SS-49 and `AvatarUpload` SS-146 are net-new) —
...
`npm run build` must produce `out/` with no error — the 27 component pages plus the
3 product screens (41 routes total).
